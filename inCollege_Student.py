...
class Student():
    ...
    def __init__(self, **kwargs):
        ...
        # last known activity initialized to now
        self.date_recently_accessed = datetime.datetime.now()
        # timer begins when student makes an account, in 7 days if no application is sent the student will be reminded upon logging in
        self.date_last_app_sent = datetime.datetime.now()
    ...
    def add_dummy_friends(self):
        student_friend_1 = Student(username='JohnSmith', firstname='John', lastname='Smith',
                                   title='Research Assistant at Harvard', about='Machine Learning Researcher')
        student_friend_1.set_education(
            'Harvard University', 'Computer Science', 'Junior')
        student_friend_1.add_job_experience(
            'job_title1', 'employer1', 'start_date1', 'end_date1', 'location1', 'description1')
        self.add_friend(student_friend_1)

        student_friend_2 = Student(username='JaneDoe', firstname='Jane',
                                   lastname='Doe', title='Software Engineering Intern at Google')
        student_friend_2.set_education(
            'Stanford University', 'Software Engineering', 'Senior')
        student_friend_2.add_job_experience('Software Engineering Intern', 'Google', '05/01/2020',
                                            '08/01/2020', 'Bay Area, LA, CA', 'Developed really cool Software at Google')
